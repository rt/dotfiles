-- Rename table and some columns

-- Old names (as view) accessed by V12.1: xout(room_type_id, xdate, id)
-- New names (as table) accessed by V12.2: blackout(room_type_id, blackout_date, blackout_id)

-- Execute the script generator ddl2rename function as shown in the example
-- below, where the array-type argument supplies the first pair as old and new
-- table names and optionally subsequent pair(s) as old and new column names.
-- The function generates a set of DDL statements to facilitate the
-- table/column name handling.

-- Copy the generated DDL output into your migration script for review and
-- eventual check-in.

-- NOTE If you are adding or removing columns to the table you are using
-- ddl2rename on you will need to modify the output such that it still is valid
-- for pre/post migration/deploy states - The script does not handle these
-- within itself. The "CASE" statement generated in the script is a good
-- starting place to review when modifying the output.
...
-- 1220000.sql.pre as a result from the ddl2rename execution above
...
-- 1220000.sql.post
--@LockTables xout
DROP TRIGGER IF EXISTS insteadof_xout_trg ON xout;
DROP FUNCTION IF EXISTS insteadof_xout_dml_fn();
DROP VIEW IF EXISTS xout;
-- Note: For column-only-rename (Scenario#2),
--+      the shadow table need to also be renamed back to the original.
-- ALTER TABLE xout_shadow RENAME TO xout;
...
-- Copy and paste the following CREATE FUNCTION statements to your test or
-- local database that has been properly migrated to a release (n-1) below the
-- target release of your change.  Prerequisite PL/pgSQL Function:
-- array_idx(anyarray,anyelement) Expand source

-- Once you have the ddl2rename function created, you may follow the usage hint
-- inside the code below, base on your scenario.

CREATE OR REPLACE FUNCTION ddl2rename(names text[], target_release int)
RETURNS SETOF text
LANGUAGE plpgsql
AS $function$
/******************************************************************************
